...
import {
  GetListEnrollmentOutput,
  GetListEnrollmentQueryParams,
  EnrollmentInput,
  GetListEventOutput,
  CreateEventInput,
  CreateEventOutput,
...
export default class ApiService {
   ...
   * @returns
   */
  static async getEnrollment(params?: GetListEnrollmentQueryParams) {
    ApiService.userIsLoggedIn()
    try {
      const response = await coreApi.get<GetListEnrollmentOutput>(apiRoutes.ENROLLMENT, {
        params: params,
      })
      //TODO: Add pagination
   ...
   * @param payload
   */
  static async updateEnrollment(payload: EnrollmentInput) {
    ApiService.userIsLoggedIn()
    try {
  ...
  static async getEvent() {
    ApiService.userIsLoggedIn()
    try {
      const response = await coreApi.get<GetListEventOutput>(apiRoutes.EVENT)
      const { results } = response.data
      return results
    } catch (error) {
      if (error.response) {
  ...
  static async getEventInstance(params?: GetEventInstanceParams) {
    ApiService.userIsLoggedIn()
    try {
      const response = await coreApi.get<GetEventInstanceOutput>(apiRoutes.EVENT_INSTANCE, {
        params: params,
      })
      const { results } = response.data
      return results
  ...
  static async createEventInstance(payload: UpdateEventInstanceInput) {
    ApiService.userIsLoggedIn()
    try {
      const response = coreApi.post<EventInstance>(apiRoutes.EVENT_INSTANCE, payload)
      return response
    } catch (error) {
      if (error.response) {
        if (error.response.status === StatusCodes.UNAUTHORIZED) {
          ...
          console.log()
          ApiService.errorMessage(
            'An error has occur: Invalid eventInstanceCode. Please contact the administrator',
          )
        } else if (error.response.status >= 500) {
          console.log(`Error Status Code: ${error.response.status} at apiRoutes.EVENT_INSTANCE`)
          console.log(error.message)
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === StatusCodes.UNAUTHORIZED) {
          ...
          console.log(
            `Error Status Code: ${error.response.status} at ${apiRoutes.EVENT_INSTANCE}/${eventInstanceCode}`,
          )
          console.log(error.message)
          ApiService.errorMessage('Backend Error, please contact the administrator')
        }
...
//   static async createFolderPermission(folderId: string, folderRole: string, username: string) {
